...
import numpy as np
@register_reranker("api_embedding")
class ApiEmbeddingReranker(BaseReranker):
    def get_similarity_score(self, s1: list[str], s2: list[str]) -> np.ndarray:
        client = OpenAI(api_key=self.config.reranker.api_embedding.key, base_url=self.config.reranker.api_embedding.base_url)
        batch_size = self.config.reranker.api_embedding.get("batch_size") or 64
        all_texts = s1 + s2
        all_embeddings = []
        for i in range(0, len(all_texts), batch_size):
            ...
            response = client.embeddings.create(
                input=batch,
                model=self.config.reranker.api_embedding.model
            )
