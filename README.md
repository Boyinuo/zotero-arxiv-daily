...
executor:
  ...
  source: ??? # The sources of papers to retrieve. Example: ['arxiv','biorxiv','ieee','sage']
...
The TLDR of each paper is generated by LLM, given the text extracted from the paper (TeX source/HTML/PDF for arXiv, abstract-only for bioRxiv/medRxiv and journal RSS sources). Papers that were already sent in previous runs are tracked in `data/sent_papers.json` and automatically skipped.
